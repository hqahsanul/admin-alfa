import { Button, ButtonGroup, Card, CardBody, CardHeader, Col, Dropdown, DropdownItem, DropdownMenu, DropdownToggle, Row } from 'react-bootstrap';
...
const DefultButtons = () => {
  return <>
          ...
          return <Button key={idx} variant={button}>
...
const OutlineButtons = () => {
  return <>
			...
			<CardBody>
				<div className="d-flex flex-wrap gap-2">
					{(extendedColorVariants || []).map((button, idx) => {
          return <Button key={idx} variant={'outline-' + button}>
								{button.charAt(0).toUpperCase() + button.slice(1)}
							</Button>;
        })}
				</div>
...
const RoundedButtons = () => {
  return <>
			...
			<CardBody>
				<div className="d-flex flex-wrap gap-2">
					{(extendedColorVariants || []).map((button, idx) => {
          return <Button key={idx} variant={button} className="rounded-pill">
								{button.charAt(0).toUpperCase() + button.slice(1)}
							</Button>;
        })}
				</div>
...
const OutlineRoundedButtons = () => {
  return <>
			...
			<CardBody>
				<div className="d-flex flex-wrap gap-2">
					{(extendedColorVariants || []).map((button, idx) => {
          return <Button key={idx} variant={'outline-' + button} className="rounded-pill">
								{button.charAt(0).toUpperCase() + button.slice(1)}
							</Button>;
        })}
				</div>
				...
				<ButtonGroup className="mb-2">
					...
					<ButtonGroup>
						<Dropdown>
							...
							<DropdownMenu>
								<DropdownItem as="a">Dropdown link</DropdownItem>
								<DropdownItem as="a">Dropdown link</DropdownItem>
							</DropdownMenu>
						</Dropdown>
					</ButtonGroup>
				...
				<Row>
					...
					<div className="col-md-3">
						<div className="btn-group-vertical mb-2">
							...
							<Dropdown>
								<DropdownToggle variant="light" data-bs-toggle="dropdown" aria-expanded="false">
									{' '}
									Button 3 <span className="caret"></span>{' '}
								</DropdownToggle>
								<DropdownMenu>
									<DropdownItem as="a">Dropdown link</DropdownItem>
									<DropdownItem as="a" className="dropdown-item">
										Dropdown link
									</DropdownItem>
								</DropdownMenu>
							</Dropdown>
...
const ToggleButton = () => {
  return <>
			<CardHeader>
				...
				<p className="text-muted mb-0">
					Add <code>data-bs-toggle=&qout;button&qout;</code> to toggle a button’s{' '}
					<code>active</code> state. If you’re pre-toggling a button, you must
					manually add the <code>.active</code> class <strong>and</strong>{' '}
					<code>aria-pressed=&qout;true&qout;</code> to ensure that it is conveyed
