...
const data = ['25', '50', '75', '100', 'auto'];
const objectTypes = ['contain', 'cover', 'fill', 'scale', 'none'];
const Utilities = () => {
  return <>
			...
			<Row>
              ...
              return <div key={idx} className={`bg-${color.variant} ${colors[idx].color ? 'text-black' : 'text-white'}  p-2 mb-2`}>
										.bg-{color.variant}
									</div>;
            })}
						</CardBody>
              ...
              return <div key={idx} className={`bg-gradient bg-${color.variant} ${colors[idx].color ? 'text-black' : 'text-white'}  p-2 mb-2`}>
					...
					<Card>
						<CardHeader>
							...
							<p className="text-muted mb-0">
								...
								<code>.text-*</code>{' '}
								<a href="https://getbootstrap.com/docs/5.3/utilities/colors/" target="_blank" rel="noreferrer">
									color utilities
								</a>
								.
							</p>
							...
							<Row>
								<Col xs={12}>
									<div className="d-flex flex-column gap-2">
										{(colors || []).slice(0, 8).map((color, idx) => {
                    return <div key={idx} className={`bg-${color.variant}-subtle p-2`}>
				...
				<Col xl={12}>
					<Card>
						...
						<CardBody>
							<div className="d-flex flex-column gap-2">
								{(colors || []).slice(0, 8).map((color, idx) => {
                return <div key={idx} className={`text-bg-${color.variant} p-2`}>
											{color.variant.charAt(0).toUpperCase() + color.variant.slice(1)}{' '}
			...
			<Row>
				<Col xl={12}>
                  ...
                  return <React.Fragment key={idx}>
												<p className={`text-${color.variant} ${colors[idx].isBG ? 'bg-dark' : ''}`}>
													.text-{color.variant}
												</p>
                ...
                return <div key={idx} className={`text-primary text-opacity-${opacity}`}>
						...
						<Card>
							...
							<CardBody>
								<div className="d-flex flex-wrap gap-2">
									{(opacities || []).map((opacity, idx) => {
                  return <div key={idx} className={`opacity-${opacity} p-2 bg-primary text-light fw-bold rounded`}>
												{opacity}%
											</div>;
                })}
								</div>
                  ...
                  return <div key={idx} className="text-center">
					...
					<Col xl={12}>
						<Card>
							...
							<CardBody>
								{(opacities || []).map((opacity, idx) => {
                return <div key={idx} className={`border border-success p-2 mb-2 border-opacity-${opacity}`}>
				...
				<Row>
                  ...
                  return <Image key={idx} src={avatar2} className={`avatar-lg rounded-${round.align}`} alt={`rounded-${round.align}`} />;
                })}
									<Image src={avatar2} className="avatar-lg rounded-circle" alt="rounded-circle" />
									<Image src={Img2} className="avatar-lg w-auto rounded-pill" alt="rounded-pill" />
								</div>
							</CardBody>
						</Card>
					</Col>
						...
						<Card>
							...
							<CardBody>
								<div className="d-flex align-items-start flex-wrap gap-2">
									...
									{(data || []).map((border, idx) => {
                  return <Image key={idx} src={avatar4} className={`avatar-lg rounded-${idx + 1}`} alt="rounded-0" />;
                })}
								</div>
							</CardBody>
						</Card>
				...
				<Row>
					<Col xl={12}>
						<Card>
							...
							<CardBody>
								{(data || []).map((width, idx) => {
                return <div key={idx} className={`w-${width} p-2 bg-light`}>
											Width {width}%
										</div>;
              })}
							</CardBody>
                ...
                height: 255
									...
									{(data || []).map((height, idx) => {
                  return <div key={idx} className={`h-${height} p-2 bg-light`}>
												Height {height}%
											</div>;
                })}
								</div>
				...
				<Row>
						...
						<Card>
							<CardHeader>
								...
								<p className="text-muted mb-0">
									...
									<a href="https://developer.mozilla.org/en-US/docs/Web/CSS/object-fit" target="_blank" rel="noreferrer">
                  ...
                  return <div key={idx}>
												<Image src={Img1} className={`object-fit-${type} border rounded avatar-xl`} alt="..." />
