import { Card, Col, Row, Badge, Button, CardHeader, CardBody } from 'react-bootstrap';
  ...
  return <>
			<Card>
				...
				<CardBody>
					...
					<h4>
						h4.Example heading{' '}
						<Link href="" className="badge bg-info-subtle text-info">
							Info Link
						</Link>
					</h4>
					<h5>
...
const PillBadges = () => {
  return <>
            ...
            return <Badge key={idx} pill bg={color} className={color === 'light' ? 'text-black' : 'text-white'}>
					...
					<div className="d-flex flex-wrap gap-1">
						{(colorVariants || []).map((color, idx) => {
            return <Badge key={idx} pill className={`bg-${color}-subtle text-${color} ${color === 'light' ? 'text-black' : ''}`}>
									{color.charAt(0).toUpperCase() + color.slice(1)}
								</Badge>;
          })}
					</div>
					...
					<div className="d-flex flex-wrap gap-1">
						{(colorVariants || []).map((color, idx) => {
            return <Badge key={idx} pill bg="" className={`badge-outline-${color} ${color === 'light' ? 'text-black ' : ''}`}>
									{color.charAt(0).toUpperCase() + color.slice(1)}
								</Badge>;
          })}
					</div>
...
const ContextualVariations = () => {
  return <Card>
          ...
          return <Badge key={idx} bg={color} className={color === 'light' ? 'text-black' : 'text-white'}>
								{color.charAt(0).toUpperCase() + color.slice(1)}
							</Badge>;
        })}
				</div>
				...
				<div className="d-flex flex-wrap gap-1">
					{(extendedColorVariants || []).map((color, idx) => {
          return <Badge key={idx} className={`bg-${color}-subtle text-${color}  ${color === 'light' ? 'text-black ' : ''}`}>
								{color}
							</Badge>;
        })}
				</div>
				...
				<div className="d-flex flex-wrap gap-1">
					{(colorVariants || []).map((color, idx) => {
          return <Badge key={idx} bg="" className={`badge-outline-${color} ${color === 'light' ? 'text-black ' : ''}`}>
								{color}
							</Badge>;
        })}
				</div>
